...
export class AlertListenerService implements OnModuleInit {
  ...
  private async handle(alerta: {
    ...
    const parque = alerta.parque_id
      ...
      : null;
    const { subject, html } = this.notif.templateAlerta(alerta, parque?.nombre);
    ...
    // Envío de email: sólo rojo o amarillo (verde se ignora por defecto)
    if (alerta.nivel === 'rojo' || alerta.nivel === 'amarillo') {
      const to = Array.from(new Set([
        ...(alerta.nivel === 'rojo' ? globalOps : []),
        ...suscritos,
      ]));
      if (to.length) await this.notif.enviarEmail(to, subject, html);
    }
